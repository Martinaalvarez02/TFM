...
def main():
    ...
    for fold, (train_idx, val_idx) in enumerate(kf.split(train_df, train_df['label'])):
        ...
        train_loader = DataLoader(H5Dataset(feats_path, sub_train_df, "train"), batch_size=batch_size, shuffle=True)
        val_loader = DataLoader(H5Dataset(feats_path, val_df, "test"), batch_size=1, shuffle=False)
    ...
    full_train_loader = DataLoader(H5Dataset(feats_path, train_df, "train"), batch_size=batch_size, shuffle=True)
    ...
    for epoch in range(num_epochs):
        ...
        for features, labels in full_train_loader:
            ...
            optimizer.step()
            total_loss += loss.item()
        print(f"Final Model - Epoch {epoch+1}/{num_epochs}, Loss: {total_loss/len(full_train_loader):.4f}")
